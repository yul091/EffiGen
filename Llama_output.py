...
class BaseModelOutputWithPastAndNorms(ModelOutput):
    ...
    Args:
        ...
        attentions (`tuple(torch.FloatTensor)`, *optional*, returned when `output_attentions=True` is passed or when `config.output_attentions=True`):
            ...
            heads.
        attention_norms (`tuple(torch.FloatTensor)`, *optional*, returned when `output_attentions=True` is passed or when `config.output_attentions=True`):
            Tuple of `torch.FloatTensor` (one for each layer) of shape `(batch_size, sequence_length)`.
            L2 norm of the attention head activations.
        mlp_norms (`tuple(torch.FloatTensor)`, *optional*, returned when `output_attentions=True` is passed or when `config.output_attentions=True`):
            Tuple of `torch.FloatTensor` (one for each layer) of shape `(batch_size, sequence_length)`.
            L2 norm of the MLP (FFN) activations.
    """
    ...
    attention_norms: Optional[Tuple[torch.FloatTensor]] = None
    mlp_norms: Optional[Tuple[torch.FloatTensor]] = None
...
class LlamaAttentionWithOutput(LlamaAttention):
...
class LlamaModelWithOutput(LlamaPreTrainedModel):
    ...
    def forward(
        ...
        return BaseModelOutputWithPastAndNorms(
            ...
            attention_norms=all_attn_norms,
            mlp_norms=all_mlp_norms,
        )
...
class LlamaForCausalLMWithOutput(LlamaPreTrainedModel):
    ...
    def forward(
        ...
        return CausalLMOutputWithPastAndNorms(
            ...
            attention_norms=outputs.attention_norms,
            mlp_norms=outputs.mlp_norms,
        )
