    ...
    def __getitem__(self, ind):
        ...
        if self.task_start_pct is None and self.task_end_pct is None:     # if fact position unspecified
            ...
            for i, text in enumerate(background_text):
                ...
                if (current_length >= task_start_ind) and (current_length < task_end_ind):
                    possible_positions.append(i)
                if current_length >= (task_end_ind - total_facts_len) and len(possible_positions) == 0:
                    print('change to other position.')
                    possible_positions.append(i)
                current_length += len(text)

            if len(possible_positions) == 0:
                print('change to final position.')
                possible_positions.append(len(background_text))
                # ipdb.set_trace()
