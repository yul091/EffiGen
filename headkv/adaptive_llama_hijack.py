...
def _reallocate_capacity(layer_head_capacity, attention_norm=None, strategy = 'reverse_norm'):
    """
    ...
    **kwargs,
    ...
    if past_key_value is not None:
        ...
        if key_states.shape[-2] == kv_seq_len: # [SnapKV] add kv_cluster
            ...
            # First calculate output norm
            layer_head_capacity = self.kv_cluster.initial_head_capacity[self.layer_idx].to(attn_output.device)
            attention_norm = torch.norm(attn_output, p=2, dim=-1)  # shape: (bsz, q_len, num_heads)
            self.kv_cluster.head_capacity[self.layer_idx] = _reallocate_capacity(
                layer_head_capacity, 
                attention_norm.mean(dim=1), 
                strategy='reverse_norm',
            )
            # self.kv_cluster.head_capacity[self.layer_idx] = _reallocate_capacity(layer_head_capacity, strategy='random')
            ...
            key_states_compress, value_states_compress = self.kv_cluster.update_kv(
                ...
                # attn_norm=attention_norm.transpose(1, 2),  # shape: (bsz, num_heads, q_len)
            )
            past_key_value.update(key_states_compress, value_states_compress, self.layer_idx, cache_kwargs)
